...
import { loadCloudState, saveCloudStateAsync } from "./cloudDb.js";
...
// Vercel Route Normalizer Middleware
app.use((req, res, next) => {
  ...
  if (req.url !== originalUrl) {
    console.log(`[Vercel Route Normalizer] Normalized ${originalUrl} to ${req.url}`);
  }
  next();
});
...
// In-Memory Database cache to prevent write/read failures on Read-Only Systems (e.g., Vercel Serverless)
    ...
    try {
      ...
      if (changed) {
        try {
          ...
          console.warn("⚠️ Read-only filesystem detected. Retaining database updates in memory only:", writeErr.message);
        }
      }
      memoryDb = db;
      return db;
...
- Hazard levels: Hazard category Class-3 toxic inhalation when vaporized under operational pressure exceeding 4 Bars.`
  ...
  try {
    ...
    console.warn("⚠️ Read-only filesystem detected during database creation. Retaining default database in memory only:", writeErr.message);
  }
  memoryDb = defaultDb;
  return defaultDb;
}
...
let isCloudInitializing = false;

// Async initializer for cloud state
async function ensureCloudDbLoaded() {
  if (isCloudLoaded || isCloudInitializing) return;
  isCloudInitializing = true;
  try {
    console.log("🌦️ [CloudDB] Initializing. Checking for Cloud Databases...");
    const cloudState = await loadCloudState();
    if (cloudState) {
      memoryDb = cloudState;
      isCloudLoaded = true;
      console.log("🌦️ [CloudDB] Successfully synchronized local cache with remote Cloud Database state!");
    } else {
      console.log("🌦️ [CloudDB] Cloud Database not active or empty. Using local disk cache/defaults.");
      isCloudLoaded = true; // Still mark as loaded to prevent duplicate slow checks
    }
  } catch (err: any) {
    console.error("❌ [CloudDB] Error loading state from cloud database:", err?.message || err);
  } finally {
    isCloudInitializing = false;
  }
}

// Read Current state
function readDb() {
  // Concurrently ensure cloud DB is synchronized in the background
  if (!isCloudLoaded && !isCloudInitializing) {
    ensureCloudDbLoaded();
  }

  // If memoryDb is already populated (from successful cloud fetch or prior in-memory request writes), return it immediately!
  // This is our single source of truth and overrides the static bundled file-system db.json.
  if (memoryDb) {
    return memoryDb;
  }

  // Otherwise, load from local disk once as immediate warm fallback
  try {
    if (fs.existsSync(DB_FILE)) {
      const fileData = fs.readFileSync(DB_FILE, "utf-8");
      const db = JSON.parse(fileData);
      
      // Basic schema validations to auto-repair empty tables
      let changed = false;
      if (!db.users) { db.users = []; changed = true; }
      if (!db.teams) { db.teams = []; changed = true; }
      if (!db.messages) { db.messages = []; changed = true; }
    ...
    console.warn("⚠️ [readDb] Read error on db.json from disk, falling back to memory:", err);
  }
  ...
  // Cold load re-initialization
  const fallbackDb = initDatabase();
  memoryDb = fallbackDb;
  return fallbackDb;
}

// Save Current state (Write-Through Cloud Sync)
function writeDb(data: any) {
  memoryDb = data;
  
  // 1. Persist to local filesystem (immediate local redundancy if writable)
  try {
    fs.writeFileSync(DB_FILE, JSON.stringify(data, null, 2), "utf-8");
  } catch (writeErr: any) {
    // Suppressed on Read-Only platforms
  }

  // 2. Concurrently dispatch background sync write to Cloud Database in an async tick
  saveCloudStateAsync(data)
    .then((result) => {
      if (result.success) {
        console.log(`🌦️ [CloudDB] Background state synchronized successfully with ${result.provider}.`);
      }
    })
    .catch((err) => {
      console.error("❌ [CloudDB] Background state synchronization failed:", err);
    });
}
...
app.post("/api/db/reset", (req, res) => {
  ...
  try {
    ...
    console.warn("⚠️ Could not delete database file, clearing memory database cache instead:", unlinkErr.message);
  }
...
app.post("/api/expert/ask", async (req, res) => {
  ...
  // --- IN-DEPTH COGNITIVE KNOWLEDGE ASSESSMENT ---
  const queryLower = query.toLowerCase();
  
  // Define keyword lists with weights
  const advancedWords = ["calibrate", "calibration", "g32", "override", "deviation", "manometer", "transducer", "amplitude", "voltage", "hercules", "siemens", "s7-300", "g-code", "baud", "parameters", "thresholds", "diagnostics", "differential", "megapascals", "micro-learning", "telemetry"];
  const intermediateWords = ["spindle", "pneumatic", "valve", "pressure", "coolant", "mechanical", "solenoid", "venting", "bar", "e-740", "err-cnc-998", "thermal", "overload", "exhaust", "manifolds", "pipeline", "suction", "indicator", "leakage", "warning", "alarm", "accesslevel", "assigneddocumentids"];
  const beginnerWords = ["help", "broken", "stop", "how to", "where is", "what does", "accident", "please", "clog", "hot", "loud", "smoke", "fire", "leak", "smell", "fix", "work"];

  const matchedAdvanced = advancedWords.filter(w => queryLower.includes(w));
  const matchedIntermediate = intermediateWords.filter(w => queryLower.includes(w));
  const matchedBeginner = beginnerWords.filter(w => queryLower.includes(w));

  // 1. Vocabulary Score Calculation
  let vocabularyScore = 0;
  vocabularyScore += matchedAdvanced.length * 25;
  vocabularyScore += matchedIntermediate.length * 15;
  vocabularyScore += matchedBeginner.length * 5;
  if (vocabularyScore > 100) vocabularyScore = 100;

  // 2. Syntactic Complexity Score Calculation
  ...
  const hasMultipleSentences = queryLower.split(/[.!?]/).filter(s => s.trim().length > 3).length > 1;
  if (hasMultipleSentences) complexityScore += 25;

  const complexTransitions = ["however", "therefore", "due to", "triggering", "exceeding", "assuming", "secondary", "consequence", "standard", "operational", "authorized"];
  const transitionsFound = complexTransitions.filter(t => queryLower.includes(t));
  complexityScore += transitionsFound.length * 10;

  // Numbers or code references (like ERR, E-, G32)
  const numbersOrCodes = /\b\d+(\.\d+)?\b|[eE]-\d+|g32|err/i.test(queryLower);
  if (numbersOrCodes) complexityScore += 20;

  if (complexityScore > 100) complexityScore = 100;

  // Define dynamic assessment level
  let assessedLevel: 'BEGINNER' | 'INTERMEDIATE' | 'ADVANCED' = 'BEGINNER';
  if (vocabularyScore >= 45 || (vocabularyScore >= 25 && complexityScore >= 65)) {
    assessedLevel = 'ADVANCED';
  } else if (vocabularyScore >= 15 || complexityScore >= 35) {
    assessedLevel = 'INTERMEDIATE';
  }

  // Ensure userPersona has priority if set manually, but if set to 'AUTO' or not provided, use assessed level
  let targetPersona: 'BEGINNER' | 'INTERMEDIATE' | 'ADVANCED' = 'INTERMEDIATE';
  if (userPersona && userPersona !== 'AUTO') {
    targetPersona = userPersona;
  } else {
    targetPersona = assessedLevel;
  }

  const detectedJargon = [...new Set([...matchedAdvanced, ...matchedIntermediate])];
  let analysisSummary = "";
  if (targetPersona === 'ADVANCED') {
    analysisSummary = `High-grade jargon density (${detectedJargon.length} items matched) and solid structural complexity detected in the query. Adapted to ADVANCED command-line instructions.`;
  } else if (targetPersona === 'INTERMEDIATE') {
    analysisSummary = `Standard operational vocabulary and process-driven phrasing detected. Adapted to INTERMEDIATE step-by-step procedure level.`;
  } else {
    analysisSummary = `Conversational, high-empathy structure detected with low technical density. Adapted to BEGINNER/high-comfort explanation style.`;
  }
  ...
  const assessmentPayload = {
    ...
    detectedJargon,
    analysisSummary,
    isDynamic: !userPersona || userPersona === 'AUTO'
  };
...
CURRENT ACTIVE PERSONA DETERMINED: [${targetPersona}]. You MUST speak exactly as required by the ${targetPersona} persona role, utilizing appropriate complexity and vocabulary matching the operator's proficiency level of ${targetPersona}.
