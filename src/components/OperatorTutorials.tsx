...
export default function OperatorTutorials({
  ...
  const availableMaterials = React.useMemo(() => {
    ...
    return assignedSources.length > 0 
      ? assignedSources 
      : documents.filter(doc => doc.accessLevel === 'OPERATOR');
  }, [documents, assignedTutorialDocIds, assignedDocumentIds, currentUserId]);

  const [selectedMaterialIds, setSelectedMaterialIds] = useState<string[]>([]);
  const [isInitialized, setIsInitialized] = useState(false);

  useEffect(() => {
    if (availableMaterials.length > 0 && !isInitialized) {
      setSelectedMaterialIds(availableMaterials.map(doc => doc.id));
      setIsInitialized(true);
    }
  }, [availableMaterials, isInitialized]);
