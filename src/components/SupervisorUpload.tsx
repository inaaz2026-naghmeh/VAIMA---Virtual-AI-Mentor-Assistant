...
export default function SupervisorUpload({ 
  ...
  }, [selectedTeamId, operatorsIdsStr]);

  // Keep selectedOp in sync with latest users prop updates
  useEffect(() => {
    if (selectedOp) {
      const freshOp = users.find(u => u.id === selectedOp.id);
      if (freshOp && freshOp !== selectedOp) {
