...
export default function SupervisorChat({
  ...
  }, [users, currentUserId, currentUserRole, teams]);
  ...
  useEffect(() => {
    if (operatorsLength > 0 && !activeOpId) {
      setActiveOpId(operators[0].id);
    }
  }, [operatorsLength, activeOpId]);

  // Set active op messages timestamp to "now" whenever new messages arrive or active op switches
  const opMessagesCount = React.useMemo(() => {
    return messages.filter(m => m.senderId === activeOpId || m.targetOperatorId === activeOpId).length;
  }, [messages, activeOpId]);
  ...
  }, [opMessagesCount, activeOpId]);

  // Scroll to bottom of active conversation room
  const totalRoomMessagesLength = messages.length;
  useEffect(() => {
    chatEndRef.current?.scrollIntoView({ behavior: 'smooth' });
  }, [totalRoomMessagesLength, activeOpId]);
