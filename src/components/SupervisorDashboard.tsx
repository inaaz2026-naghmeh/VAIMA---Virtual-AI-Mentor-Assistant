...
export default function SupervisorDashboard({ 
  ...
  // Date filter states: default to May 15, 2026 to the current date dynamically (or June 5 as minimum baseline)
  const [startDate, setStartDate] = useState<string>('2026-05-15');
  const [endDate, setEndDate] = useState<string>(() => {
    const today = new Date();
    const todayStr = today.toISOString().split('T')[0];
    return todayStr > '2026-06-05' ? todayStr : '2026-06-05';
  });
  ...
  // Pre-filter datasets based on selected active dates and selected team configuration using timezone-neutral UTC boundaries
  ...
  const generateChartData = () => {
    // Generate dates list from startDate to endDate using UTC boundaries 
    ...
    while (curr <= end && protectionLimit < 500) {
    ...
    <div id="analytics-dashboard-root" className="space-y-6">
      ...
      {subTab === 'ai-engine' ? (
        ...
        <div className="flex flex-wrap items-center gap-3">
          {(currentUserRole === 'MANAGER' || currentUserRole === 'SUPERVISOR') && teams && teams.length > 0 && (
