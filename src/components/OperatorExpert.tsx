    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      {currentUser?.role !== 'OPERATOR' && (
        ...
        <div>
          ...
          {/* Live Operator Knowledge Assessor Widget */}
          {assessment && !loading && (
            <div id="cognitive-assessor-panel" className="bg-[#111] border border-[#262626] rounded-xl p-4 mt-3 mb-4 space-y-3 shadow-md">
              <div className="flex items-center justify-between border-b border-[#1f1f1f] pb-2">
                <div className="flex items-center gap-1.5">
                  <Brain className="w-4.5 h-4.5 text-amber-500 animate-pulse" />
                  <span className="font-sans font-bold text-xs text-[#ededed] uppercase tracking-wider">
                    Dynamic Knowledge Assessor
                  </span>
                </div>
                <span className="text-[9px] font-mono text-zinc-500 bg-[#0a0a0a] px-1.5 py-0.5 rounded border border-[#1f1f1f]">
                  {assessment.isDynamic ? "DYNAMIC AUTO-MODE" : "MANUAL PRESET ACTIVE"}
                </span>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Left Column: Proficiency Badge & Summary */}
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-mono text-zinc-400">Assessed Skill Level:</span>
                    <span id="assessed-level-badge" className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold tracking-wider ${
                ...
                {/* Right Column: Quantitative metrics & Jargon tags */}
                <div className="space-y-2">
                  {/* Progress bars */}
                  <div className="space-y-1.5">
                    <div>
                      <div className="flex justify-between text-[10px] font-mono mb-0.5">
                        <span className="text-zinc-400">Terminology & Technical Vocabulary</span>
                        <span className="text-zinc-300 font-bold">{assessment.vocabularyScore}%</span>
                      </div>
                      <div className="h-1 bg-zinc-900 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full ${
                            assessment.level === 'ADVANCED' ? 'bg-amber-500' : assessment.level === 'INTERMEDIATE' ? 'bg-blue-500' : 'bg-emerald-500'
                          }`}
                          style={{ width: `${assessment.vocabularyScore}%` }}
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex justify-between text-[10px] font-mono mb-0.5">
                        <span className="text-zinc-400">Structural Sentence Phrasing</span>
                        <span className="text-zinc-300 font-bold">{assessment.complexityScore}%</span>
                      </div>
                      <div className="h-1 bg-zinc-900 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full bg-zinc-400`}
                          style={{ width: `${assessment.complexityScore}%` }}
                        />
                      </div>
                    </div>
                  </div>

                  {/* Jargon tag chips if any */}
                  {assessment.detectedJargon.length > 0 && (
                    <div className="flex flex-wrap items-center gap-1.5 pt-1">
                      <span className="text-[9px] font-mono text-zinc-500">Jargon Hits:</span>
                      {assessment.detectedJargon.map((term, i) => (
                        <span key={i} className="text-[9px] font-mono bg-zinc-900 border border-zinc-800 text-zinc-450 px-1.5 py-0.5 rounded">
                          {term}
                        </span>
                      ))}
                    </div>
